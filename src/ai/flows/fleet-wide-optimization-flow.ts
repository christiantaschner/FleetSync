...
**Core Principle:** Do NOT make changes for minor improvements. Only suggest a change if it solves a clear problem (like fitting in a high-priority job) or offers a substantial net profit gain across the fleet (e.g., saving significant travel time for a high-cost technician, or enabling a more profitable job to be completed). Customer satisfaction is paramount; avoid moving fixed appointments.
...
4.  **Look for Geographic & Financial Clusters**: Identify if unassigned jobs are very close to a skilled, lower-cost technician's existing route. Suggest inserting these jobs if it doesn't create significant delays for their confirmed appointments and is profitable.
5.  **Calculate Other Impacts**: For each suggested change, also calculate:
...
6.  **Justify Every Change**: Provide a clear, concise \`justification\` explaining the benefit, starting with the financial impact. Examples: "Increases net profit by $55 by assigning to a closer, lower-cost technician.", "Assigns a high-priority job while only slightly reducing margin.", "Frees up a specialized technician for a more complex job later.".
7.  **Summarize Overall Benefit**: Provide a high-level \`overallReasoning\`. If changes are made, summarize the net benefit (e.g., "Assigns 2 high-priority jobs and reduces total travel time by 45 minutes, increasing total margin by $150.").
